...
def get_files():
        ...
        if dirName != '.' and dirName != os.path.join(".","test") and dirName != os.path.join(".","training"):
            if dirName.startswith(os.path.join(".","training","")):
                ...
                for fname in fileList:
                    if '.hea' in fname:
                        header_loc.append(dirName + os.sep + fname)
                    if '-arousal.mat' in fname:
                        arousal_loc.append(dirName + os.sep + fname)
                    if 'mat' in fname and 'arousal' not in fname:
                        signal_loc.append(dirName + os.sep + fname)

            elif dirName.startswith(os.path.join(".","test","")):
    ...
    tr_ind = list(np.where(df.is_training.values)[0])
    print("TRIND PRINT", tr_ind)
    te_ind = list(np.where(df.is_training.values == False)[0])
